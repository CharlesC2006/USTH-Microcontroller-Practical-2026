...
int16_t read_temperature_sensor()
{
    i2c_start();
    bool acknowledged =
        i2c_write(TEMP_SENSOR_ADDR << 1);
    acknowledged =
        i2c_write(0xAC) && acknowledged;
    acknowledged =
        i2c_write(0x33) && acknowledged;
    acknowledged =
        i2c_write(0x00) && acknowledged;
    i2c_stop();
    if (!acknowledged)
    {
        return -127;
    ...
    uint32_t raw_temp =
        (((uint32_t)(data3 & 0x0F)) << 16) |
        (((uint32_t)data4) << 8) |
        data5;
    int16_t temp_c =
        (int16_t)((raw_temp * 200UL) / 1048576UL) - 50;
    return temp_c;
}
void setup()
{
